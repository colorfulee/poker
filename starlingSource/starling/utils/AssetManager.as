...
package starling.utils
{
    ...
    public class AssetManager extends EventDispatcher
    {
        ...
        public function addTexture(name:String, texture:Texture):void
        {
            ...
            {
                log("Warning: name "+name +" was already in use; the previous texture will be replaced.");
        ...
        public function addTextureAtlas(name:String, atlas:TextureAtlas):void
        {
            ...
            if (name in mAtlases)
            {
                log("Warning: name "+name +" was already in use; the previous atlas will be replaced.");
		...
		private var _runtimeLoadTexture:Dictionary;//运行时临时加载的纹理，单独缓存起来以便清理
		...
		public function loadTexture(path:String,callBack:Function,isCache:Boolean=true):void{
			...
			if(_runtimeLoadTexture == null) _runtimeLoadTexture = new Dictionary();
			...
			function onLoaderComplete(event:Object):void{
				...
				completeAssets(bitmap);
			}
			
			function completeAssets(asset:Object):void{
				if (asset is Bitmap){
					texture = Texture.fromData(asset, mDefaultTextureOptions.clone());
					texture.root.onRestore = function():void{
						mNumLostTextures++;
						loadRawAsset(path, null, function(asset:Object):void{
				...
				bytes.clear();
				if(isCache) _runtimeLoadTexture[name] = texture;
				completeLoadIist();
			}
			
			function atfOnReady():void{
				if(callBack != null) callBack(texture);
			}
		...
		public function clearRuntimeLoadTexture():void{
			if(_runtimeLoadTexture != null){
				for each (var texture:Texture in _runtimeLoadTexture) texture.dispose();
