...
//		}
		...
		{
			WindowFactory.showResourceWindow();
			
			FileLoader.getInstance().onProgress.add(updateConfigProgress);
		 ...
		 * 
		 */		
		private function updateConfigProgress(value:Number):void
		{
			SignalCenter.onShowProgress.dispatch("配置素材加载中:"+Number(value * 100).toFixed() +'%');
		}
		/**
		 * 更新过程 
		 * @param uint
		 * 
		 */		
		private function updateAssetProgress(value:Number):void
		{
			SignalCenter.onShowProgress.dispatch("素材加载中:"+Number(value * 100).toFixed() +'%');
		}
		...
		public function onInitialAssetsLoadComplete(cbo:ConfigVO):void {
			...
			FileLoader.getInstance().onProgress.remove(updateConfigProgress);
			
			FileLoader.getInstance().onProgress.add(updateAssetProgress);
...
//									this[cbo.id].setData(cbo.data);
			...
			if(max == 0) {
//				loadJson();'
				FileLoader.getInstance().onProgress.remove(updateAssetProgress);
				
				AssetsLoader.getAssetManager().enqueue(AssetsCenter.getFullPath("assets_tw/effects/20050/20050.bytes"));
				AssetsLoader.getAssetManager().enqueue(AssetsCenter.getFullPath("assets_tw/effects/20050/20050.xml"));
				AssetsLoader.getAssetManager().enqueue(AssetsCenter.getFullPath("assets_tw/effects/20050/20050.png"));
				
				AssetsLoader.getAssetManager().enqueue(AssetsCenter.getFullPath("assets_tw/effects/win/win.png"));
				AssetsLoader.getAssetManager().enqueue(AssetsCenter.getFullPath("assets_tw/effects/win/win.xml"));
				AssetsLoader.getAssetManager().enqueue(AssetsCenter.getFullPath("assets_tw/effects/win/win.bytes"));
//				AssetsLoader.getAssetManager().enqueue(AssetsName.EFFECT_WIN,["assets/assets_tw/effects/win/"],50);
//					AssetsLoader.getAssetManager().enqueue((50003),[file.resolvePath(formatString(AssetsCenter.ASSETS+"effects/chat/"+(50003),STLConstant.scale))],50);
		...
		{ 
			if(p == 1)
			{
				WindowFactory.hideResourceWindow();
				SignalCenter.onInitialLoadComplete.dispatch();
//				loadJson();
			}
			SignalCenter.onShowProgress.dispatch("特效素材加载中:"+Number(p * 100).toFixed() +'%');
		}
...
//		}
		...
		private function jsonEnoughHandler(e:Event):void
		{
			...
			loader.removeEventListener(Event.COMPLETE, jsonEnoughHandler);
